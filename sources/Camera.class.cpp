...
/*   Updated: 2018/03/23 06:04:40 by rmc-coma         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Camera::Camera(void) : AComponent(*(new GameObject(*(new Scene(*(new Window("kek", 0, 0, 0, 0, 0)))), "kek")), CMP_CAMERA)
{
	return ;
}

Camera::Camera(const GameObject &gameobject) : AComponent(gameobject, CMP_CAMERA),
											   _FOV(70),
											   _Near(0.1f),
											   _Far(1000.0f),
											   _ViewportX(0),
...
Camera::Camera(const GameObject &gameobject, const GLfloat fov, const GLfloat near,
			   ...
			   const GLuint viewportwidth, const GLuint viewportheight) : AComponent(gameobject, CMP_CAMERA),
																		  _FOV(fov),
																		  _Near(near),
																		  _Far(far),
...
Camera::Camera(const Camera &other) : AComponent(other._GameObject, other._Type)
{
	*this = other;
	return ;
}
